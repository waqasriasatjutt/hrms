...
patch(ListContainer.prototype, {
    ...
    get isPlusIconVisible() {
        const user = this.pos?.user;
        return user && !user.hide_pos_new_order_button;
    },
});
...
patch(ListContainer, {
    template: xml`
        <div class="overflow-hidden d-flex flex-grow-1" t-attf-class="{{props.class}}">
            <button t-if="props.onClickPlus and isPlusIconVisible" class="list-plus-btn btn btn-secondary btn-lg me-1" t-on-click="props.onClickPlus">
