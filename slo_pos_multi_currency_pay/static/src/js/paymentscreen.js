...
currency.push(this.pos.currency.currency_params)
    ...
    compute_currency(ev){
            ...
            if(currency_id){
                ...
                var total_value = parseFloat(total_array[1].replace(/,/g, ''))
                $('.rate_string')[0].innerText = current_currency.rate_string
                //Total in converted curency
                var display_total = formatMonetary(current_currency.rate * total_value)
                if(display_total == 0){
                    $('.conversion_container').css({'display':'none'})
                }else{
                    // $('.total_amount')[0].innerText = display_total
                    let display_total_currency = current_currency.symbol ?
                                               `${display_total} ${current_currency.symbol} ` :
                                               `${display_total} ${current_currency.name}`;

                    $('.total_amount')[0].innerText = display_total_currency;
                }
            }else{
                $('.conversion_container').css({'display':'none'})
            }
