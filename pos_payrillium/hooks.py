...
def show_payrillium_wizard_once(env):
    ...
    """
    _logger.info("  Running show_payrillium_wizard_once (post-init hook)")
    config = env['payrillium.config'].search([], limit=1)
    if not config or not config.token or not config.installed:
        _logger.info("  No Payrillium config found, creating server action to show wizard")
        ...
        _logger.info("  Payrillium config with token already exists, wizard will not be shown")


def uninstall_cleanup_payrillium(env):
    """
    Cleanup hook that runs during module uninstallation
    """
    _logger.info("  Running uninstallation cleanup for pos_payrillium")
    ...
    try:
        ...
        open_sessions = env['pos.session'].search([
            ...
            ('config_id.payment_method_ids.use_payment_terminal', '=', 'payrillium')
        ])
        
        if open_sessions:
            session_names = ', '.join(open_sessions.mapped('name'))
            _logger.error(f"  Cannot uninstall: open POS sessions using Payrillium exist: {session_names}")
            raise UserError(f"  Cannot uninstall module: there are open POS sessions using Payrillium.\nPlease close these sessions first:\n{session_names}")
        # 1. FIRST Find and disable ALL payment providers that might be Payrillium
        _logger.info("  Searching for all possible Payrillium providers...")
        providers = env['payment.provider'].search([
            '|',
            ('code', '=', 'payrillium'),
            ('name', 'ilike', 'Payrillium'),
        ])
        
        if not providers:
            _logger.warning("   No providers found with code 'payrillium', searching by name...")
            providers = env['payment.provider'].search([
                ('name', 'ilike', 'Terminal'),
            ])

        if providers:
            _logger.info(f"    Found {len(providers)} payment providers to process")
            for provider in providers:
                _logger.info(f"    Processing provider: {provider.name} (state: {provider.state})")
                
                # Just disable the provider
                if provider.state in ['enabled', 'test']:
                    provider.write({'state': 'disabled'})
                    _logger.info(f"  Provider {provider.name} disabled successfully")
        ...
        if pos_methods:
            _logger.info(f"    Deactivating {len(pos_methods)} POS payment methods")
            for method in pos_methods:
                method.write({'active': False})
                _logger.info(f"  POS payment method deactivated: {method.name}")

        # 3. Remove payment method lines
        journal = env['account.journal'].search([('code', '=', 'PAYR')], limit=1)
        if journal:
            method_lines = env['account.payment.method.line'].search([('journal_id', '=', journal.id)])
            if method_lines:
                _logger.info(f"  Removing {len(method_lines)} payment method lines")
                method_lines.unlink()
        ...
        # 5. Archive the PAYR journal (preserve accounting entries)
        if journal:
            ...
            moves_count = env['account.move'].search_count([('journal_id', '=', journal.id)])
            if moves_count > 0:
                _logger.info(f"  Journal has {moves_count} posted accounting entries, archiving journal instead of deleting")
            
            _logger.info("    Archiving PAYR journal")
            journal.write({
                'active': False,
                'name': f"{journal.name} (Archived - Payrillium Uninstalled)"
            })
