...
def get_terminals_from_token(full_token):
    ...
    masked_token = f"{full_token[:4]}****" if full_token and len(full_token) > 4 else "****"
    _logger.info(
...
def create_payment_token(record, payload_data):
    ...
    try:
        ...
        if not contains_card:
            log_payrillium_event(
                execution_id, "tokenization/createToken", "request", payload_data)
        else:
            _logger.info(
        ...
        log_payrillium_event(
            execution_id, "tokenization/createToken", "response", data, success=True)
        ...
        log_payrillium_event(execution_id, "tokenization/createToken",
                             "response", body, success=False, error_message=message)
        _logger.error(
