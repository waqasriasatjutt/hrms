...
class ProductTemplate(models.Model):
    ...
    body_type = fields.Selection([
        ...
        ('Pickup Truck (Single Cabin)', 'Pickup Truck (Single Cabin)'),
        ('Pickup Truck (Double Cabin)', 'Pickup Truck (Double Cabin)'),
        ('Off-Road / Jeep / 4x4', 'Off-Road / Jeep / 4x4'),
        ('Crew Cab Pickup', 'Crew Cab Pickup'),
        ('Chassis Cab', 'Chassis Cab'),

        ('MPV (Multi-Purpose Vehicle)', 'MPV (Multi-Purpose Vehicle)'),
        ('Minivan', 'Minivan'),
        ('Van', 'Van'),
        ('Cargo Van', 'Cargo Van'),
        ...
        ('Roadster', 'Roadster'),
        ('Targa Top', 'Targa Top'),
        ('Grand Tourer (GT)', 'Grand Tourer (GT)'),

        ('Limousine', 'Limousine'),
        ('Hearse', 'Hearse'),
        ('Ambulance', 'Ambulance'),
        ('Panel Van', 'Panel Van'),
        ('Ute (Car-Pickup Hybrid)', 'Ute (Car-Pickup Hybrid)'),

    ], string='Body Type', help='Complete car body type classification')
    ...
    def action_mark_as_sold(self):
        ...
        return {
            ...
            'res_model': 'car.sale.invoice.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
...
#     )
    ...
    car_expense_ids = fields.One2many(
        'car.expense',
        'car_id',
        string="Car Expenses"
    )
    ...
    @api.depends('car_expense_ids')
    def _compute_expense_count(self):
        for record in self:
            record.expense_count = len(record.car_expense_ids)
    ...
    def _sync_car_attributes(self):
        ...
        for product in self:
            for field_name, attribute_title in CAR_ATTRIBUTE_MAP.items():
                field_value = product[field_name]

                if not field_value:
                    continue  # skip empty fields

                # 1. Create or get attribute
                attr = product._get_or_create_attribute(attribute_title)

                # 2. Create or get attribute value
                val = product._get_or_create_attribute_value(attr, str(field_value))

                # 3. Create product.template.attribute.line
                ...
                if not existing_line:
                    ...
                    existing_line.value_ids = [(6, 0, [val.id])]

    def _get_or_create_attribute(self, attribute_name):
        Attribute = self.env["product.attribute"]
        attr = Attribute.search([("name", "=", attribute_name)], limit=1)
        if not attr:
    ...
    def _get_or_create_attribute_value(self, attr, value_name):
    ...
    def create(self, vals):
        ...
        product._sync_car_attributes()
        return product

    def write(self, vals):
        res = super().write(vals)
        self._sync_car_attributes()
        return res


    @api.model
    def create(self, vals):
        # If target_price is set, make list_price the same
        if vals.get('target_price'):
            vals['list_price'] = vals['target_price']
        return super().create(vals)

    def write(self, vals):
        # If target_price is updated, update list_price as well
