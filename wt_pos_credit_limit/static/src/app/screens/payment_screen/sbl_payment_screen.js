...
patch(PaymentScreen.prototype, {
    async addNewPaymentLine(paymentMethod) {
        ...
        if(paymentMethod.credit_journal && !this.currentOrder.partner_id) {
        ...
        if (paymentMethod.credit_journal && totalAmountDue > partnerAvailableCredit) {
    ...
    updateSelectedPaymentline(amount = false) {
        ...
        if(payment_method.credit_journal && !this.currentOrder.partner_id) {
            return this.dialog.add(AlertDialog, {
                title: _t("Customer Required"),
                body: _t(
                    "Please select a customer before adding a credit payment line."
    ...
    deleteCreditPaymentLines() {
        const paymentLines = this.paymentLines.filter(
            (line) => line.payment_method_id?.credit_journal
        );
